...
export default {
  ...
  mounted() {
    axios.get('http://120.25.164.172:9124/system/shift/').then(res => {
      ...
      this.tableData = this.tableData.map(row => {
        // console.log(row.end)
        // if (row.end < this.getNowFormatDate()) {
  ...
  methods: {
    ...
    tableRowClassName({ row, rowIndex }) {
      this.compareDate(this.getNowFormatDate(), row.row.end)
      console.log(row.end)
      if (this.compareDate(this.getNowFormatDate(), row.row.end)) {
        // return 'warning-row'
        console.log('kjdsfhkjsdh')
      }
      return ''
    },
    getNowFormatDate() {
